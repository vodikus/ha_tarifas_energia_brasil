...
This document describes the technical architecture of the Tarifas de Energia Brasil custom component for Home Assistant.

## Directory Structure

```text
custom_components/tarifas_energia_brasil/
...
│   ├── __init__.py          # Exports TarifasDeEnergiaBrasilDataUpdateCoordinator
...
│   ├── __init__.py          # Exports TarifasDeEnergiaBrasilEntity
...
- `base.py` - Main coordinator class (`TarifasDeEnergiaBrasilDataUpdateCoordinator`)
...
**Key class:** `TarifasDeEnergiaBrasilDataUpdateCoordinator` (exported from `coordinator/__init__.py`)
...
**Key class:** `TarifasDeEnergiaBrasilApiClient`
...
- `TarifasDeEnergiaBrasilConfigFlowHandler` (main flow)
- `TarifasDeEnergiaBrasilOptionsFlow` (options)
...
**Key class:** `TarifasDeEnergiaBrasilEntity` (in `entity/base.py`)
...
2. `TarifasDeEnergiaBrasilEntity` for common functionality
...
1. Create directory: `custom_components/tarifas_energia_brasil/<platform>/`
2. Implement `__init__.py` with `async_setup_entry()`
3. Create entity classes inheriting from platform base + `TarifasDeEnergiaBrasilEntity`
