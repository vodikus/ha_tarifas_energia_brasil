...
- Files: `snake_case.py`
- Classes: `PascalCase` prefixed with `TarifasDeEnergiaBrasil`
...
- Inherit from both platform entity and `TarifasDeEnergiaBrasilEntity` (order matters)
...
from custom_components.tarifas_energia_brasil.sensor import async_setup_entry
...
**Config entry data:** `entry_data: TarifasDeEnergiaBrasilData = hass.data[DOMAIN][entry.entry_id]`
