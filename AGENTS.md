...
- **Repository:** vodikus/ha_tarifas_energia_brasil

**Key directories:**

- `custom_components/tarifas_energia_brasil/` - Main integration code
...
- Integration logs: `custom_components.tarifas_energia_brasil: debug` in `config/configuration.yaml`
...
- Use the domain `tarifas_energia_brasil` for all DOMAIN references
- Prefix all integration-specific classes with `TarifasDeEnergiaBrasil`
- Use "Tarifas de Energia Brasil" as the display title
...
- Inherit from platform base + `TarifasDeEnergiaBrasilEntity`
...
- `tests/` mirrors `custom_components/tarifas_energia_brasil/` structure
