...
**Directory:** `custom_components/tarifas_energia_brasil/[platform]/`
...
```python
"""[Platform] platform for Tarifas de Energia Brasil."""
...
from .[entity_file] import TarifasDeEnergiaBrasil[EntityName]
from .const import DOMAIN
from .coordinator import TarifasDeEnergiaBrasilDataUpdateCoordinator
...
async def async_setup_entry(
    ...
    coordinator: TarifasDeEnergiaBrasilDataUpdateCoordinator = hass.data[DOMAIN][
        entry.entry_id
    ]

    async_add_entities(
        [
            TarifasDeEnergiaBrasil[EntityName](coordinator, entry),
...
```python
"""[Entity description] for Tarifas de Energia Brasil."""
...
class TarifasDeEnergiaBrasil[EntityName](
    TarifasDeEnergiaBrasilEntity,
    [PlatformEntityClass],
):
    ...
    def __init__(
        self,
        coordinator: TarifasDeEnergiaBrasilDataUpdateCoordinator,
        entry: ConfigEntry,
    ) -> None:
...
Add platform to `custom_components/tarifas_energia_brasil/manifest.json`:
...
- [ ] Entity class inherits from both `TarifasDeEnergiaBrasilEntity` and platform class
...
- **Class prefix:** `TarifasDeEnergiaBrasil`
- **Base entity:** `TarifasDeEnergiaBrasilEntity` in `entity/base.py`
- **Coordinator:** `TarifasDeEnergiaBrasilDataUpdateCoordinator`
